...
//---------------------------------------------Fin de los métodos para la obtencion de los atributos--------------------------------
    ...
    protected String detectarDireccionamiento(String[] codigos){
        String idenDireccionamiento = "";
            ...
            }else if(dirExt(codigos[2]) || isEtiqueta(codigos[2])){//Con esta condicion sabemos si sera directo o extendido
    ...
    protected void opcionesModos(String abreviatura, String[] codigos){        
        switch(abreviatura){        //Recibe como variable el modo que identifico
            ...
            case "DEX":
                ...
                if(isEtiqueta(codigos[2]))
                {
                    setModo("EXT");
                }
                else if(!"null".equals(idenBase(codigos[2])))
                {
    ...
    /**
     * Método que nos ayuda a identificar en que base esta el operando y nos regresa su equivalente en hexadecimal
     * @param identificador
     * @return 
     */
    ...
    protected static boolean baseHexa(String hexa){
        return Pattern.compile("[0-9A-Fa-f]+").matcher(hexa).matches();
    }//Fin del método base hexadecimal
    /**
     * Método utilizado para saber el tamaño del operando ya sea de 16 bits o de 8 bits
     * @param cifra 
     */
    protected void nBits(String cifra){
           String rellenar = "0";           
           cifra = quitarCeros(cifra);
     ...
     * @return 
     */
    protected String quitarCeros(String cifra){
        ...
        return cifra;                
    }//Fin del método para eliminar los ceros a la izquierda
//-----------------------------------------Modos de direccionamiento indexado------------------------------------------------
    ...
    protected String separarCadena(String operando,String inicial,int valor){
        ...
        if(inicial.equals(""))
        {//Inicio del if inicialque nos ayuda a identificar si no tiene corchetes 
            if(valor>0)
            {
        ...
        }//Fin del if para saber si la instruccion tuvo corchetes o no
        else
        {//Inicio de else para cuando si hubo corchetes 
            if(valor>0)
            {
        ...
        }//Fin del else para separar la cadena cuando hubo corchetes                     
    ...
    protected void modosIndexados(String operando){
        ...
        {//Inicio del else 1°     
            ...
            {   //If para cuando el modo de direccionamiento indexado es indirecto ya que se han detectado los corchetes #2.111
                switch (indeIndirectos(operando)) {
                //fin del if si el direccionamiento es indexado indirecto de 16bits
                    case "[IDX2]":
                        //inicio del if si el direccionamiento es indexado indirecto de 16bits
                        setModo("[IDX2]");
                        break;
                //fin del else if si el direccionamiento es indexado indirecto de registro d
                    case "[D,IDX]":
                        //inicio del if else si el direccionamiento es indexado indirecto de registro d
                        setModo("[D,IDX]");
                        break;
                    default:
                        //inicio de else por si existe un error
                        setError(false);
                        System.out.println("ERROR en el operando\n");
                        break; //fin del else para mandar mensaje de error
                }
            }//fin del if para cuando el modo de direccionameinto indexado es indirecto [   ]  #2.111
            ...
            {//inicio del else if para revisar si el direccionamiento es indexado de regsitros a, b o d
            ...
            }//Fin del else if del direccionamiento indexado de registro a, b o d
            else 
            {
                setError(false);
                System.out.println("ERROR en el operando\n");
